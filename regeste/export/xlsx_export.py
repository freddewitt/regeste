...
def export_xlsx(pieces: list[Piece], output_path: Path, *, validated_only: bool = False) -> Path:
    pieces = filter_pieces(pieces, validated_only=validated_only)
    wb = Workbook()
    overview = wb.active
    overview.title = "Vue d'ensemble"
    _write_sheet(overview, pieces)
    ...
    for key, group_pieces in groups.items():
        ...
        _write_sheet(sheet, group_pieces)
...
def _write_sheet(sheet: Worksheet, pieces: list[Piece]) -> None:
